...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        model = StageNet_II(76+17, 384, 10, 1, 3, 0.3, 0.3, 0.3).to(device)
        optimizer = torch.optim.Adam(model.parameters(), lr=args.lr)
        
        print("loading pre-trained StageNet-II model")
        checkpoint = torch.load('./saved_weights/trained_model_ablation2.zip', map_location = device)
