...
if __name__ == '__main__':
    ...
    if args.test_mode == 1:
        ...
        if args.file_name == 'StageNet':
            print("loading orignal StageNet pre-trained model")
            checkpoint = torch.load('./saved_weights/StageNet', map_location = device)
        elif args.file_name == 'trained_model':
            print("loading reproduced StageNet pre-trained model")
            checkpoint = torch.load('./saved_weights/trained_model.zip', map_location = device)
        else:
            print("Please set args.file_name to 'StageNet' for original author's pre-trained model or 'trained_model' for reproduced pre-trained model")
