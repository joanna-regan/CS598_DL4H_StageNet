...
Start with:
StageNet II: only has stage-aware LSTM. 
"""
import torch
...
class StageNet_II(nn.Module):
    ...
    def step(self, inputs, c_last, h_last, interval):
        ...
        c_in = torch.tanh(x_out[:, self.levels*3:])   #Eq 2.1
    ...
    def forward(self, input, time, device):
        ...
        output = self.nn_output(origin_h) 
