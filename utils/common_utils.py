...
class DeepSupervisionDataLoader:
    ...
    def __init__(self, dataset_dir, listfile=None, small_part=False):
        ...
        mappings_path = os.path.join(base_dir, "mappings.csv")
        
        with open(mappings_path, "r") as mfile:
            loaded_mappings = mfile.readlines()
            
        loaded_mappings = [line.split(',') for line in loaded_mappings]
        loaded_mappings = pd.DataFrame(loaded_mappings, columns = ['stay','subgroup'])

        self._data = [line.split(',') for line in self._data]
        self._data = [(x, float(t), y) for (x, t, _, y) in self._data]
        ...
        while i < len(self._data):
            ...
            #print("cur_Stay: " + cur_stay)
            #print("length of loaded mappings " + cur_stay)
            #print("listfile_path " + cur_stay)
            
            cur_X, header = self._read_timeseries(cur_stay, loaded_mappings, listfile_path)
            mas["X"].append(cur_X)
            ...
            i = j
            if small_part > 0:
                if 'train' in listfile_path and len(mas["name"]) == small_part:
                    break
                if 'val' in listfile_path and len(mas["name"]) >= small_part * 0.1112:
                    break
                if 'test' in listfile_path and len(mas["name"]) >= small_part * 0.2778:
                    break

        self._data = mas

    def _read_timeseries(self, ts_filename, mappings, listfile_path):
        ret = []
        
        if 'test' in listfile_path:
            open_path = os.path.join(self._dataset_dir, ts_filename)
        else:
            cur_subgroup = mappings[mappings.stay == ts_filename].subgroup.item().replace("\n","")
            open_path = os.path.join(self._dataset_dir, cur_subgroup, ts_filename)    
        
        with open(open_path, "r") as tsfile:
